...
parser.add_argument('--wd', '--weight_decay', default=0.0001, type=float, metavar='WEIGHT_DECAY',
...
def parse_and_set_args(block):
    ...
    # scale lr linearly according to batch size
...
def get_train_and_valid_data_loaders(block, args):
    ...
    train_dataset = args.dataset_class(args, root=args.train_file, is_training=True)
...
def load_model(model, optimizer, block, args):
    ...
    # load partially trained weights
    model_state = model.state_dict()
    ckpt_state = checkpoint["state_dict"]
    for model_param, ckpt_param in zip(model_state, ckpt_state):
        if model_param == ckpt_param:
            if 'weight' in model_param and model_state[model_param].shape[1] != ckpt_state[ckpt_param].shape[1]:
                size = model_state[model_param].shape
                zeros = torch.zeros((size[0], \
                                    model_state[model_param].shape[1] - ckpt_state[ckpt_param].shape[1],
                                    size[2], size[3]), requires_grad=True)
                torch.nn.init.xavier_uniform_(zeros, gain=1.0)
                model_state[model_param] = torch.cat((ckpt_state[ckpt_param], zeros), dim=1)
            else:
                model_state[model_param] = ckpt_state[ckpt_param]
    ...
    #    args.start_epoch = 1#max(0, checkpoint['epoch'])
...
def train_step(batch_cpu, model, optimizer, block, args, print_linf_grad=False):
    # Move data to GPU.
    inputs = {k: [b.cuda() for b in batch_cpu[k]] for k in batch_cpu if k == "image"}
...
def save_model(model, optimizer, epoch, global_index, block, args):
    ...
    if args.rank == 0:
        ...
        model_name = os.path.join(
            args.save_root, '_ckpt_epoch_%03d_iter_%07d.pth' % (
                epoch, global_index))
...
def train(model, optimizer, lr_scheduler, train_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            save_model(model, optimizer, epoch + 1, global_index, block, args)
...
def main():
    ...
    # logfile
    logfile = os.path.join(args.name, 'logs.dat')

    # write config
    with tools.TimerBlock("Write train config", logfile, args.rank) as block:
        write_config(block, args)

    # Initialize torch.distributed.
    with tools.TimerBlock("Initializing Distributed", logfile, args.rank):
        initialilze_distributed(args)

    # Set all random seed for reproducability.
    with tools.TimerBlock("Setting Random Seed", logfile, args.rank):
        set_random_seed(args.seed)

    # Train and validation data loaders.
    with tools.TimerBlock("Building {} Dataset".format(args.dataset), logfile, args.rank) as block:
    ...
    with tools.TimerBlock("Building {} Model and {} Optimizer".format(
            args.model, args.optimizer_class.__name__), logfile, args.rank) as block:
    ...
    with tools.TimerBlock("Building {} Learning Rate Scheduler".format(
            args.optimizer), logfile, args.rank) as block:
    ...
    # Start training
    with tools.TimerBlock("Training Model", logfile, args.rank) as block:
